...
export function checkInteractions(newMedicineName, existingNames) {
  const results = [];
  for (const interaction of interactions) {
    ...
    const matchExisting = interaction.drugs.some((d) =>
      existingNames.some((e) => e.toLowerCase().includes(d.toLowerCase()) || d.toLowerCase().includes(e.toLowerCase()))
    );
    if (matchExisting) results.push(interaction);
  }
  return results;
}

export function checkAllInteractions(medicineNames) {
  const results = [];
  for (let i = 0; i < medicineNames.length; i++) {
    for (let j = i + 1; j < medicineNames.length; j++) {
      for (const interaction of interactions) {
        ...
        if (matchA && matchB) results.push({ ...interaction, between: [medicineNames[i], medicineNames[j]] });
      }
    }
  }
  return results;
